...
local currentFilter = nil  -- v11.14: when set (lowercased name), show ONLY the current head/hair; nil = normal
local programmaticText = false -- v11.14: true while WE set searchInput.Text, so its OnChange won't clear currentFilter
...
local function passes(e)
    if currentFilter ~= nil and e.lname ~= currentFilter then return false end   -- v11.14 current-only filter (no-op when nil)
...
function CCNS_Refresh()
    ...
    CCNS_Mods = mods
    selectedMods = {}
    currentFilter = nil   -- v11.14: a full Refresh clears the current-only filter
...
rebuildModChecks = function()
    ...
    for _, mod in ipairs(CCNS_Mods) do
        ...
        cb.OnChange = function(h) if programmaticText then return end currentFilter = nil; if h.Checked then selectedMods[modName] = true else selectedMods[modName] = nil end refreshDisplay() end
...
-- v11.14: find the LIVE-collection index (== the baked grid "#i") of a given DisplayName
-- handle, so the CURRENT item's true thumbnail number is correct even when names repeat.
local function currentTileIndex(dc, collProp, handle)
    local coll = readColl(dc, collProp)
    if not coll then return nil end
    local n = 0; pcall(function() n = #coll end)
    for i = 1, (n or 0) do
        local it; pcall(function() it = coll[i] end)
        local hh; if it then pcall(function() hh = it.Name end) end
        if hh and tostring(hh) == handle then return i end
    end
    return nil
end

-- v11.14: "Show current" -- filter the list to ONLY the currently-equipped head/hair (per the
-- active slot) and report its true thumbnail number. Purely read + IMGUI filter: it reads the
-- live SelectedHair/SelectedHead, never writes game state, and every failure path returns
-- BEFORE touching currentFilter so a failed press never leaves the list stuck-filtered.
local function showCurrent()
    local cc = acquireCC()
    if not cc then if setStatus then setStatus("Open the Head or Hair screen first, then try again.") end return end
    local isHair   = (cc.slot == "Hair")
    local selProp  = isHair and "SelectedHair" or "SelectedHead"
    local collProp = isHair and "SelectableHair" or "SelectableHeads"
    local sel = readColl(cc.dc, selProp)
    local h; if sel then pcall(function() h = sel.Name end) end
    if not h then if setStatus then setStatus("Couldn't read the current " .. tostring(cc.slot) .. " selection - press Refresh, then retry.") end return end
    h = tostring(h)
    if h == "" or h == UNKNOWN then if setStatus then setStatus("Couldn't identify the current " .. tostring(cc.slot) .. ".") end return end
    if #CCNS_Entries == 0 then CCNS_Refresh() end
    local nm = stripPrefixes(firstLine(Ext.Loca.GetTranslatedString(h) or "")); if nm == "" then nm = "(unnamed)" end
    local lname = string.lower(nm)
    local hasRow = false
    for _, e in ipairs(CCNS_Entries) do if e.lname == lname then hasRow = true; break end end
    if not hasRow then if setStatus then setStatus("Current " .. tostring(cc.slot) .. " (" .. nm .. ") isn't in the list yet - press Refresh, then try again.") end return end
    local tileNum = currentTileIndex(cc.dc, collProp, h)
    -- clear the OTHER filters so only the current item shows (mirrors "Show all mods").
    -- programmaticText guards the WHOLE block so neither the checkbox nor the text OnChange
    -- (if BG3SE ever fired them on a programmatic set) can clear currentFilter.
    programmaticText = true
    selectedMods = {}
    for _, m in ipairs(modCheckboxes) do pcall(function() m.cb.Checked = false end) end
    if searchInput then pcall(function() searchInput.Text = "" end) end
    programmaticText = false
    query = ""
    currentFilter = lname   -- set LAST so nothing above can clear it
    if refreshDisplay then refreshDisplay() end
    local tail = tileNum and ("  ->  thumbnail #" .. tileNum) or "  (number appears after a Head<->Hair tab round-trip)"
    if setStatus then setStatus("Current " .. tostring(cc.slot) .. ": " .. nm .. tail) end
end
...
local function buildPanel()
    ...
    local ok, err = pcall(function()
        ...
        searchInput.OnChange = function(hh) if programmaticText then return end currentFilter = nil; query = string.lower(hh.Text or ""); refreshDisplay() end
        ...
        local curBtn = win:AddButton("Show current Head/Hair")
        curBtn.OnClick = function() showCurrent() end
        ...
        local clr = win:AddButton("Show all mods"); clr.OnClick = function()
            currentFilter = nil; selectedMods = {}; for _, m in ipairs(modCheckboxes) do pcall(function() m.cb.Checked = false end) end refreshDisplay()
        end
...
log("CCNS v11.14 loaded (HOTKEY-ONLY: Ctrl+Shift+H toggles the search panel; open gated to the Hair/Head picker via one on-demand walk; no Tick heartbeat, no auto-open, no per-tick logging; labels + mod-of-origin; on-demand tile numbering of ALL tiles incl. vanilla; showCurrent() Show-current-Head/Hair button; CCNS_Search()/CCNS_Toggle(); v11.14 = v11.13 + Show current head/hair button).")
