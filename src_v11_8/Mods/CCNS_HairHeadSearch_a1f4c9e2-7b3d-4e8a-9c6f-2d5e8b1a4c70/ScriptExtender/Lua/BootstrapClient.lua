...
-- onto MODDED hair/head tiles only, by rewriting their DisplayName loca string to
-- "#i <name>\n<mod>". Pure loca, no XAML. Base/vanilla tiles (no entry in the
-- handle->mod map) are left untouched. Reversible: loca edits are per-session, so
-- a vanilla relaunch restores originals. Returns the count baked.
    ...
    local function numberColl(coll)
        ...
        for i = 1, (n or 0) do
            ...
            if h and h ~= "" and h ~= UNKNOWN then
                local mod = CCNS_HandleMod[h]          -- modded-only; nil = base/vanilla -> skip
                if mod then
                    local cur = Ext.Loca.GetTranslatedString(h) or ""
...
local function buildPanel()
    ...
    local ok, err = pcall(function()
        ...
        win.Open = false   -- shown only in CC (GameStateChanged gate)
...
Ext.Events.StatsLoaded:Subscribe(function()
    ...
    buildPanel()       -- IMGUI panel (once); hidden until in CC
end)

-- Show the panel ONLY while in character creation. We can't rely on a CC
-- component event (the old hook never fired), so we re-check on each coarse
-- GameStateChanged: if the picker walk finds the CC view-model we're in CC
-- (open + load); otherwise we've left it (clear stars + hide). Single
-- lightweight listener, no per-tick polling. (Flagged deviation from R2,
-- whose named hook does not exist; this is the clean replacement.)
-- The CC appearance screen builds AFTER the "Running" state change and there is
-- no event for it; Ext.Timer.WaitFor proved unreliable here. So on ANY
-- GameStateChanged we open a bounded poll WINDOW, and a single gated
-- Ext.Events.Tick handler checks acquireCC a few times/sec during that window,
-- opening the panel the instant the hair/head picker exists. While open, the
-- same tick does a slow presence-check and auto-closes once the picker is gone.
-- The tick body is a no-op when idle (near-zero cost). acquireCC is the same
-- read-only walk used by manual open/apply.
local inCC = false
local pollUntil = 0     -- monotonic-ms deadline for the open-poll window; 0 = idle
local lastPoll = 0
local miss = 0
local function autoTick()
    local t = now()
    if pollUntil ~= 0 then
        if t > pollUntil then pollUntil = 0; return end
        if (t - lastPoll) < 350 then return end
        lastPoll = t
        if acquireCC() then
            pollUntil = 0; miss = 0
            if not inCC then inCC = true; openPanel(); CCNS_Refresh(); log("auto-open: picker found -> panel opened") end
        end
    elseif inCC then
        if (t - lastPoll) < 1200 then return end
        lastPoll = t
        if acquireCC() then miss = 0
        else miss = miss + 1; if miss >= 2 then inCC = false; miss = 0; closePanel(); log("auto-close: left character creation") end end
    end
end
pcall(function() Ext.Events.Tick:Subscribe(function() pcall(autoTick) end) end)
pcall(function()
    Ext.Events.GameStateChanged:Subscribe(function(e)
        log("GameState " .. tostring(e and e.FromState) .. " -> " .. tostring(e and e.ToState))
        if inCC then inCC = false; miss = 0; pcall(closePanel) end
        pollUntil = now() + 45000; lastPoll = 0
    end)
end)
...
local function togglePanel()
    ...
    if not win then log("hotkey: panel object missing"); return end
    local open = false; pcall(function() open = win.Open end)
    if open then pcall(function() win.Open = false end); log("hotkey: panel hidden")
    else
        pcall(function() win:SetPos({ 40, 90 }, "Always") end)   -- force visible spot
        pcall(function() win.Open = true end)
        CCNS_Refresh()
        log("hotkey: panel shown")
    end
end
...
log("CCNS v11.7 loaded (search tile #; brighter ring; Tick auto-open; Ctrl+Shift+H hotkey, NO key suppression; MODDED grid-tile numbering #i on refresh + CCNS_NumberProbe(); CCNS_Search()/CCNS_Toggle()).")
