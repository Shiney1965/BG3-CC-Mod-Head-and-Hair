...
> **New in 1.1.0 — Show current Head/Hair.** The search panel now has a **Show current Head/Hair** button that filters the list to the option you *currently have equipped* — the current hair on the Hair screen, the current head on the Head screen — and shows its exact thumbnail number, so you can find the active style in the grid without hunting for the gold-outlined tile. Clear it again with **Refresh**, by typing in the search box, by toggling a source-mod filter, with **Show all mods**, or by reopening the panel.
...
- **Show current Head/Hair** — click this button to filter the list down to the hair or head you currently have equipped and read off its thumbnail number, so you can locate the active style in the grid. Clear the filter with **Refresh**, by typing, by toggling a source-mod filter, with **Show all mods**, or by reopening the panel.
...
- **Show current Head/Hair** *(new in 1.1.0)* — a panel button that filters the list to the hair or head you currently have equipped (per the screen you're on) and reports its true thumbnail number, so you can find the active option in the grid without scrolling for the gold-outlined tile.
...
- **Show current Head/Hair:** click the button to filter the list to the option you currently have equipped and read its thumbnail number; clear it with **Refresh**, typing, a source-mod filter, **Show all mods**, or reopening the panel.
...
- The **search panel itself is a keyboard/mouse feature**: it opens with `Ctrl+Shift+H` and is operated with the mouse (it is not navigable with a controller). Because the numbering is baked when you open the panel, numbering is likewise keyboard/mouse-driven. The **Show current Head/Hair** button is part of that panel and is therefore keyboard/mouse only as well.
...
At launch, a Script Extender Lua client script builds a map from each character-creation appearance visual to the mod that supplied it (using the engine's static-data sources), then bakes the option's name, tile number, and source mod into the text the tile displays. The same data drives the search panel, which reads the live hair/head collections and applies your selection back to the game. The **Show current Head/Hair** button reads the currently-equipped hair/head from that same live collection and matches it to its tile number. The panel is summoned only by `Ctrl+Shift+H`, and the keypress checks that you're on the Hair/Head picker before opening — there is no always-on background polling.
...
**Q: How do I find the hair or head I already have on?**
A: Open the panel and click **Show current Head/Hair**. It filters the list to your currently-equipped option (the hair on the Hair screen, the head on the Head screen) and shows its thumbnail number, so you can find it in the grid. Clear the filter with **Refresh**, by typing, by toggling a source-mod filter, with **Show all mods**, or by reopening the panel.
